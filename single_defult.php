	...
	<div class="content singledefult">
		<div class="postcontent singledefult" id="post-<?php the_ID(); ?>" <?php post_class(); ?>>		
			<div class="blogpost">		
				...
				<?php if(has_tag()) { ?>
					<?php if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { $tagsTR = stripText($data['translation_tags']); } else {  $tagsTR = __('Tags: ','wp-musica'); } ?>
		...
		if ($query_custom->have_posts()) : ?>
			<div class="relatedtitle">
				<h3><?php if (!function_exists('icl_object_id') or (ICL_LANGUAGE_CODE == $sitepress->get_default_language()) ) { echo stripText($data['translation_relatedpost']); } else {  _e('Related post','wp-musica'); } ?></h3>
			</div>
			<div class="related">	
			...
			while ($query_custom->have_posts()) : $query_custom->the_post(); 
				if ( has_post_thumbnail() ){
					$image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'related', false);
					$image = $image[0];
					}
				else
				...
				<?php } ?>
						<div class="image"><a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title(); ?>"><img src="<?php echo $image ?>" alt="<?php the_title(); ?>"></a></div>
