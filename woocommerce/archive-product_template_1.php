					...
					if($countPost != 2){
						...
						<?php if(isset($postmeta["video_active"][0]) && $postmeta["video_active"][0] == 1) { ?>
							<div class="recentimage">
								<div class="image">
									...
									<?php
										...
										{  
											echo '<iframe class = "productIframe withsidebar" src="http://player.vimeo.com/video/'.$postmeta["video_post_url"][0].'" width="288" height="230"  ></iframe>';  
										}  
										else if ($postmeta["selectv"][0] == 'youtube')  
										{  
											echo '<iframe class = "productIframe withsidebar youtube"  width="288" height="230" src="http://www.youtube.com/embed/'.$postmeta["video_post_url"][0].'"  ></iframe>';  
										}  
										else  
										{  
							...
							<div class="recentimage">
								...
								<div class="image">
									<div class="loading"></div>
									<?php if (has_post_thumbnail( get_the_id() )) echo get_image_pmc(288,230,get_the_id()); else echo '<img src="'.woocommerce_placeholder_img_src().'" alt="Placeholder" width="'.$woocommerce->get_image_size('shop_catalog_image_width').'px" height="'.$woocommerce->get_image_size('shop_catalog_image_height').'px" />'; ?>
								</div>
							</div>						
							<?php  }  ?>
						</a>
							<div class="recentdescription">
								...
								<h3><a href="<?php echo get_permalink( get_the_id() ) ?>" title="<?php the_title() ?>"><?php echo substr(the_title(' ', ' ', false),0,99) ?></a></h3>
								<?php 
								if ( $count > 0 ) :
								...
								?>
								<div class="shortDescription"><?php echo shortcontent('[', ']', '', get_post_field('post_content', get_the_id() ) ,90);?> ... <?php closeTags(shortcontent('[', ']', '', get_post_field('post_content', get_the_id() ) ,95)) ?></div>
