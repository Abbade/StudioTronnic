...
function optionsframework_admin_init() {
		...
		global $of_options_musica_pmc, $options_machine;




		$options_machine = new Options_Machine($of_options_musica_pmc);
...
<div class="wrap" id="of_container">
  ...
  <form id="of_form" method="post" action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" enctype="multipart/form-data" >
    <div id="header">
      <div class="logo">
        <h2>Musica - Premium Wordpress Theme</h2> 
      </div>
	...
	foreach ($options as $value) {
		...
		case 'slider':
				...
				if ($count < 2) {
					$oldorder = 1;
					$order = 1;
					$output .= Options_Machine::optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order,$int,$value['descshow'],$value['team'],$value['nivo'],$value['social'] = false);
				} else {
					$i = 0;
				...
				$output .= '<a href="#" class="button slide_add_button_desc">Add New</a></div>';			
				}			
				if($value['social'] == true){
				$output .= '<a href="#" class="button slide_add_button_social">Add New Social Icon</a></div>';			
				}			
				if($value['ios'] || $value['nivo']){
				$output .= '<a href="#" class="button nivo_slide_add_button">Add New Slide</a></div>';			
				}						
